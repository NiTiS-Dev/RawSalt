...
public struct Mat3x3
{
	...
	public Mat3x3(ReadOnlySpan<float> data)
	{
		if (data.Length < ElementCount)
			throw new ArgumentOutOfRangeException(nameof(data));

		this = Unsafe.ReadUnaligned<Mat3x3>(ref Unsafe.As<float, byte>( ref MemoryMarshal.GetReference(data)));
	}

	public Mat3x3(ReadOnlySpan<byte> data)
	{
		if (data.Length < sizeof(float) * ElementCount)
			throw new ArgumentOutOfRangeException(nameof(data));

		this = Unsafe.ReadUnaligned<Mat3x3>(ref MemoryMarshal.GetReference(data));
	}

	public static Mat3x3 Identity
	{
		get {
			Mat3x3 retusa = default;
